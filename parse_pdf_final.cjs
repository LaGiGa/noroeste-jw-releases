...
async function parse() {
    ...
    for (let i = 0; i < meetingIndices.length; i++) {
        ...
        // 2. Find "footer names" - names that appear BEFORE the date line
        // These are typically the last 4-6 lines before the date
        const footerNames = [];
        for (let j = dateIndex - 1; j >= Math.max(0, dateIndex - 10); j--) {
            ...
            // Valid name
            if (line.length > 2 && !line.includes('Vida e Ministério') && !line.includes('Impresso')) {
                footerNames.unshift(line); // Add to beginning to maintain order
            }
        }

        console.log(`  Footer names found: ${footerNames.length} - ${footerNames.join(', ')}`);
        ...
        // 4. Collect names that appear between headers and numbered parts (for Ministry - Principal room)
        const namesBlock = [];
        ...
        // Assign footer names to treasures parts (first 2 footer names)
        ...
        // 8. Find Sala B names for Ministry parts
        ...
        // 9. NVC parts (7+) - excluding Bible Study
        const nvcParts = [];
        meetingLines.forEach((l, idx) => {
            ...
            if (m && !l.includes('Estudo Bíblico de Congregação')) {
                ...
                // If no name on same line, try footer names
                if (!name && footerNames.length > treasuresParts.length) {
                    const footerIdx = treasuresParts.length + (num - 7);
        ...
        // 10. Oração Final - use last footer name
