...
async function parse() {
    ...
    for (let i = 0; i < meetingIndices.length; i++) {
        ...
        // 1. MAIN BLOCK (Previous Date -> Current Date) - Principal Names & Some Parts
        const mainBlock = lines.slice(prevBlockStart, dateIndex);

        // 2. NEXT BLOCK (Current Date -> Next Date) - Sala B Names & Other Parts
        let nextDateIndex = (i === meetingIndices.length - 1) ? lines.length : meetingIndices[i + 1];
        const nextBlock = lines.slice(dateIndex + 1, nextDateIndex);

        // Update prevBlockStart
        prevBlockStart = dateIndex + 1;

        // --- IDENTIFY PARTS (Combined Blocks) ---
        // Collect first occurrence of each part number 1..8
        const combinedBlock = [...mainBlock, ...nextBlock.slice(0, 40)]; // Limit Next block to avoid grabbing WAY next week parts
        const allPotentialParts = [];
        combinedBlock.forEach((l, idx) => {
            const m = l.match(/^(\d+)\.\s+([^(]+)/);
            if (m) allPotentialParts.push({ num: parseInt(m[1]), title: m[2].trim().replace(/:$/, ''), line: l });
        });

        const allParts = [];
        const seenNums = new Set();
        allPotentialParts.forEach(p => {
        ...
        const bibleStudy = allParts.find(p => p.line.includes('Estudo Bíblico')); // Safe logic
        // Better Bible Study logic: find line starting with 8. or 9. containing Estudo
        const bibleStudyPart = allParts.find(p => p.title.includes('Estudo Bíblico'));
        ...
        ministryParts.forEach(p => {
            // Only assign if it's NOT a Discurso (Part 6 sometimes Discurso, check logic)
            // But we have names for 4, 5, 6 usually.
            if (principalNames[ptr]) {
                meeting.assignments.push({ studentName: principalNames[ptr], point: p.title, room: 'Principal' });
                ptr++;
            }
        });
        ...
        // --- EXTRACT SALA B NAMES (from Next Block) ---
        const salaBCount = ministryParts.length;
        const salaBCandidates = nextBlock.filter(l => isValidName(l) && !l.includes('Leitura da Bíblia') && !l.includes('Estudo Bíblico'));
        ...
        if (bibleReading) {
            ...
            // Try to find Sala B line in combined block?
            // Usually it's in Next Block or end of Main Block?
            // Week 1: Reading is Part 3 (Main Block or Next Block? Lines 46 - Next Block).
            // Sala B line (Line 47 - Next Block).

            // Search in the block where Reading Line was found +/- 5 lines
            const contextBlock = combinedBlock; // simplified
            const rIdx = contextBlock.indexOf(readingLine);

            // ... manual search is safer
            const salaBLine = combinedBlock.slice(Math.max(0, rIdx - 5), rIdx + 10).find(l => l.includes('Sala B:'));
        ...
        // Bible Study
        if (bibleStudyPart) {
            const studyLine = bibleStudyPart.line;
